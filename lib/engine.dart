...
class Engine {
  ...
  final List<int> _barrier = List.filled(COL_COUNT, -1);
  ...
  Stream<Tetrimino> get playerStream =>
      _playerController.stream.flatMap((tetramino) {
        ...
        return RangeStream(0,
          ...
          final _nextPiece = Tetrimino(
              current: _piece.current,
              origin: _piece.origin,
              position: _piece.position + 1);
          final _nextIndexes = mapToGridIndex(_nextPiece, extent, COL_COUNT);
          return _nextIndexes.any((item) => _barrier.contains(item));
        }).doOnData((piece) {
          _current = piece;
        }).doOnDone(() {
          _setPieces.add(_current);
          final _currentIndexes = mapToGridIndex(_current, extent, COL_COUNT);
  ...
  Stream<List<TetrisUnit>> get gridStateStream =>
      _gridController.stream.map((List<Tetrimino> setPieces) {
        final _setIndexes = <TetrisUnit>[];
        for (final piece in setPieces) {
          _setIndexes.addAll(mapToGridIndex(piece, extent, boardWidth ~/ extent)
              .map((item) => TetrisUnit(index: item, color: piece.color!)));
        }
        return _setIndexes;
      });
